    ...
    return (
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {[1, 2, 3, 4].map(item => (
            <div key={item} className="bg-gray-100 rounded-lg p-4 h-24 animate-pulse"></div>
          ))}
        </div>
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="p-4 border-b border-gray-200 flex flex-wrap gap-2">
            {[1, 2, 3, 4, 5].map(item => (
              <div key={item} className="h-8 bg-gray-200 rounded-full w-20 animate-pulse"></div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-yellow-500">
          ...
          <p className="text-2xl font-bold text-gray-900">
            {getStatusCount("confirmed") + getStatusCount("pending") + getStatusCount("in-progress")}
          </p>
        </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden mb-6">
        <div className="p-4 border-b border-gray-200 flex flex-wrap gap-2">
          <button
            ...
            className={`px-3 py-1 rounded-full text-sm font-medium ${filter === "all" ? "bg-blue-100 text-blue-800" : "bg-gray-100 text-gray-800 hover:bg-gray-200"}`}
          >
            All ({bookings.length})
          </button>
          <button
            onClick={() => setFilter("pending")}
            className={`px-3 py-1 rounded-full text-sm font-medium ${filter === "pending" ? "bg-gray-100 text-gray-800" : "bg-gray-100 text-gray-800 hover:bg-gray-200"}`}
          >
            Pending ({getStatusCount("pending")})
          </button>
          <button
            onClick={() => setFilter("confirmed")}
            className={`px-3 py-1 rounded-full text-sm font-medium ${filter === "confirmed" ? "bg-blue-100 text-blue-800" : "bg-gray-100 text-gray-800 hover:bg-gray-200"}`}
          >
            Confirmed ({getStatusCount("confirmed")})
          </button>
          <button
            onClick={() => setFilter("in-progress")}
            className={`px-3 py-1 rounded-full text-sm font-medium ${filter === "in-progress" ? "bg-yellow-100 text-yellow-800" : "bg-gray-100 text-gray-800 hover:bg-gray-200"}`}
          >
            In Progress ({getStatusCount("in-progress")})
          </button>
          <button
            onClick={() => setFilter("completed")}
            className={`px-3 py-1 rounded-full text-sm font-medium ${filter === "completed" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800 hover:bg-gray-200"}`}
          >
            Completed ({getStatusCount("completed")})
          </button>
          <button
            onClick={() => setFilter("cancelled")}
            className={`px-3 py-1 rounded-full text-sm font-medium ${filter === "cancelled" ? "bg-red-100 text-red-800" : "bg-gray-100 text-gray-800 hover:bg-gray-200"}`}
          >
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                  <div className="flex-1">
                    <div className="flex items-center">
                      ...
                      <span className={`ml-3 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${statusColor(booking.status)}`}>
                  ...
                  <div className="mt-4 flex items-center">
                    <div className="flex items-center">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <svg
                          key={star}
                          className={`h-5 w-5 ${star <= booking.rating! ? 'text-yellow-400' : 'text-gray-300'}`}
                          fill="currentColor"
                          viewBox="0 0 20 20"
                        >
                    ...
                    <span className="ml-2 text-sm text-gray-500">You rated this service</span>
                  </div>
                )}
              </div>
            ))}
