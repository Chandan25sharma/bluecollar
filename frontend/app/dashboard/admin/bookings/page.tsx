...
export default function AdminBookingsPage() {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      setIsLoading(true);
      try {
        const response = await adminAPI.getAllBookings(page, 20, statusFilter || undefined);
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
          <div className="flex flex-wrap gap-4">
            ...
            <div className="text-sm text-gray-600 flex items-center">
              Total Bookings: <span className="font-semibold ml-1">{bookings.length > 0 ? `${(page - 1) * 20 + bookings.length}+` : 0}</span>
            </div>
          </div>
        </div>
